...
NexT.boot.registerEvents = function() {
    ...
    // Mobile top menu bar.
    document.querySelector('.site-nav-toggle .toggle')
        .addEventListener('click', event => {
            event.currentTarget.classList.toggle('toggle-close');
            const siteNav = document.querySelector('.site-nav');
            if (!siteNav) return;
            siteNav.style.setProperty(
                '--scroll-height', siteNav.scrollHeight + 'px');
...
NexT.boot.refresh = function() {
     ...
     * Need to add config option in Front-End at
     * 'scripts/helpers/next-config.js' file.
     */
...
NexT.boot.motion = function() {
    ...
    if (CONFIG.motion.enable) {
        NexT.motion.integrator.add(NexT.motion.middleWares.header)
